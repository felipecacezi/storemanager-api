...
describe('Get all services cases tests', () => {
    ...
    it('should return all services', async () => {
        ...
        const services = await getAllServicesUseCase.execute({ page: 1, limit: 10 });
    ...
    it('should throw an error if page is not provided', async () => {
        try {
            await getAllServicesUseCase.execute({ page: 0, limit: 10 });
        } catch (error) {
    ...
    it('should throw an error if limit is not provided', async () => {
        try {
            await getAllServicesUseCase.execute({ page: 1, limit: 0 });
        } catch (error) {
            expect(error).to.be.an('error');
