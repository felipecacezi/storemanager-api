...
export class VendorController {
    ...
    async create(request: FastifyRequest, reply: FastifyReply) {
        ...
        await this.createVendorUseCase.execute({ name, email, document, phone, is_whatsapp, country, address, number, city, neighborhood, state, zipcode, complement });
    ...
    async update(request: FastifyRequest, reply: FastifyReply) {
        ...
        await this.updateVendorUseCase.execute({ id: Number(id), ...data });
        return reply.status(200).send({ success: true, message: "Fornecedor atualizado com sucesso!" });
    }

    async delete(request: FastifyRequest, reply: FastifyReply) {
        const { id } = request.params as any;
        await this.deleteVendorUseCase.execute({ id: Number(id) });
        return reply.status(200).send({ success: true, message: "Fornecedor removido com sucesso!" });
    }

    async getAll(request: FastifyRequest, reply: FastifyReply) {
        const { page, limit } = request.query as any;
        const vendors = await this.getAllVendorsUseCase.execute({ page: Number(page), limit: Number(limit) });
